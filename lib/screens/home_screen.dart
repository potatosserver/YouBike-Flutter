...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
  ...
  void _moveMapToStation(Station station) {
    ...
    void listener() {
      ...
      final currentLat = start.latitude + (target.latitude - start.latitude) * t;
      final currentLng = start.longitude + (target.longitude - start.longitude) * t;
      _mapController.move(LatLng(currentLat, currentLng), 18.0);
        ...
        children: [
          if (appState.center != null)
            Positioned(
              ...
              left: 0,
              right: 0,
              bottom: (_panelHeight ?? screenHeight * 0.35) + 12, 
              child: ClipRRect(
                ...
                child: FlutterMap(
                  ...
                  options: MapOptions(
                    ...
                    onPositionChanged: (pos, hasMoved) {
                      if (appState.isFollowingUser && pos.center != null) {
            ...
            child: Column(
              children: [
                ...
                Expanded(
                  child: Container(
                    decoration: BoxDecoration(
                      color: theme.brightness == Brightness.dark ? const Color(0xFF2D2A25) : const Color(0xFFFFFBEB),
    ...
    return Center(
      child: GestureDetector(
        onTap: appState.isUpdating ? null : () {
          appState.searchStations(''); // 取消任何待處理的搜尋防抖計時器
          appState.refreshStations(reason: "USER_UPDATE");
        },
        child: Container(
          padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
          decoration: BoxDecoration(
            color: theme.brightness == Brightness.dark ? const Color(0xFF4A453C) : const Color(0xFFFFD700),
